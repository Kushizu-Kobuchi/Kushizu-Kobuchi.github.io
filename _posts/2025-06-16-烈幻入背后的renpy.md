...
对话框里显示的话其实都是某个角色说出的话，角色是`Character()`构造出的实例，可以设置名字、类型、图片等等。这里贴一下实现。动态角色对象允许在剧本里动态地改变名字，每次对话前，`DynamicCharacter`都会计算`name_expr`的值并修改名字。
...
三年前我一直有个遗憾，那就是没有给千亦搞一个彩虹色的轮廓线，这两天我在查资料时发现并不难实现——果然这些东西就是会者不难难者不会。我先留一个[参考链接](https://lemmasoft.renai.us/forums/viewtopic.php?p=565136)，等后面再详细说。
